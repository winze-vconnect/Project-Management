...
const DeviceLocationPlanner = () => {
  ...
  const filteredLocations = locations.filter((loc) =>
    (loc.unit_name || '').toLowerCase().includes(search.toLowerCase()) ||
    (loc.city || '').toLowerCase().includes(search.toLowerCase()) ||
    (loc.entity_name || '').toLowerCase().includes(search.toLowerCase())
  );

  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold text-blue-700 mb-2">Device Location Planner</h1>
      <ExcelUpload onUploadSuccess={fetchLocations} />
          ...
          <tbody>
            {filteredLocations.map((loc, index) => (
              <tr key={loc.id} className="hover:bg-orange-50">
                ...
                <td className="p-2 border">{loc.unit_name}</td>
                <td className="p-2 border">{loc.city}</td>
                <td className="p-2 border">{loc.entity_name}</td>
                <td className="p-2 border">{loc.unit_hr_name}</td>
